...
# 3 - Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
        tableau6Caractere = 0
        emails.each do |mail|
            if mail.length == 6  #6 parce que @ - 1 = 5 car ont a pas besoin de @
                tableau6Caractere += 1
            end
          end
    puts " il y a #{tableau6Caractere} qui ont 5 caractere " 
                      
    [1,2,3,4,5,6].select(&:even?)

    vjournal = emails.select{|k| k.length == 5}  

    puts vjournal.count

# 4 - Combien commencent par une majuscule (première lettre juste après le @) ?
if la 1-Lettre .upcase 
  alors tu me le met dans lettreEnMaj
lettremaj=0


  emails.each do |email|
    if email[1]==email.upcase
      lettremaj += 1 
...
puts " il y a #{lettremaj} relou qui ont une lettre maj lol "
...
def tab1
  ...
  def tab(journalists)
    puts "To see the number of journalists, you type 'journalists.count' or 'journalists.length'."
    journalists.length
